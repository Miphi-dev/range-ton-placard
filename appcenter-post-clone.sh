...
if [ "$APPCENTER_XCODE_PROJECT" ]; then
  ...
  echo $GOOGLE_SERVICE | base64 --decode > "$APPCENTER_SOURCE_DIRECTORY/ios/GoogleService-Info.plist"
else
  # Si c'est un projet Android
  echo "Copie du contenu de GOOGLE_SERVICE dans google-services.json"
  echo $GOOGLE_SERVICE | base64 --decode > "$APPCENTER_SOURCE_DIRECTORY/android/app/google-services.json"
fi
